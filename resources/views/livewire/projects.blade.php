<div class="bg-white lg:min-w-0 lg:flex-1">
    ...
    <div class="pl-4 pr-6 pt-4 pb-4 border-b border-t border-gray-200 sm:pl-6 lg:pl-8 xl:pl-6 xl:pt-6 xl:border-t-0">
      ...
      <h1 class="flex-1 text-lg leading-7 font-medium">{{ $page }}</h1>
        ...
        <div class="relative">
          ...
          <div class="origin-top-right z-10 absolute right-0 mt-2 w-56 rounded-md shadow-lg hidden">
            <div class="rounded-md bg-white shadow-xs">
              <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="sort-menu">
                <a href="#" class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900" wire:click="changeOrder('name')" role="menuitem">Name</a>
                <a href="#" class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900" wire:click="changeOrder('created_at')" role="menuitem">Date modified</a>
                <a href="#" class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900"  wire:click="changeOrder('category')" role="menuitem">Category</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <ul class="relative z-0 divide-y divide-gray-200 border-b border-gray-200" wire:poll.3000ms>
        ...
        <div class="flex items-center justify-between space-x-4">
          ...
          <div class="hidden sm:flex flex-col flex-shrink-0 items-end space-y-3">
            <p class="flex items-center space-x-4">
              <a href=" {{ $project->uri }}" class="relative text-sm leading-5 text-gray-500 hover:text-gray-900 font-medium">
                Visit site
              </a>
              <button class="relative" type="button" wire:click="addStar({{ $project->id }})">
                <svg class="h-5 w-5 text-yellow-300 hover:text-yellow-400" viewBox="0 0 20 20"  fill="@if($project->starred == 1)currentColor @endif">
            ...
            <span> {{ $project->category }}</span>
              <span>&middot;</span>
                <span>Last deploy {{ $project->created_at->diffForHumans() }}</span>
              <span>&middot;</span>
